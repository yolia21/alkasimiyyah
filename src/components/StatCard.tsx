...
export default function StatCard({ label, value, subtitle, icon }: StatCardProps) {
  return (
    <div className="relative overflow-hidden rounded-xl border border-ivory-300 bg-white p-6 shadow-sm hover:shadow-md hover:border-brass-gold-400 transition-all duration-300 group">
      ...
      <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-ottoman-red-700 via-brass-gold-400 to-ottoman-red-700 opacity-80 group-hover:opacity-100 transition-opacity duration-300" />
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <h3 className="text-2xl font-bold text-ottoman-red-900 font-serif leading-tight">
            {value}
          </h3>
          {subtitle && (
        ...
        {icon && (
          <div className="p-2.5 rounded-lg bg-ivory-100 text-ottoman-red-800 group-hover:bg-ottoman-red-50 group-hover:text-ottoman-red-700 transition-colors duration-300">
            {icon}
          </div>
        )}
      </div>
