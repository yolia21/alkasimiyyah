...
const ISSUE_TYPES = ["Infrastructure Concern","Security Incident","Administrative Complaint","Provincial Boundary Matter","Public Safety","Cultural or Religious Matter","Financial or Treasury Concern","Request for Government Services","Interpersonal Dispute","Other"];
const PROVINCES = ["Wilayat Ghabaan","Wilayat Jama'ah","Cross-Provincial","Capital (Ismailabad)","External / Overseas"];

export default function ReportPage() {
  const [form, setForm] = useState({ civilId:"", fullName:"", issueType:"", province:"", severity:"", description:"", desiredOutcome:"" });
  const [status, setStatus] = useState<"idle"|"submitting"|"success"|"error">("idle");

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault(); setStatus("submitting");
    const payload = { access_key: WEB3FORMS_KEY, subject: `[Kasimid Portal] Issue Report (${form.severity}) — ${form.civilId}`, from_name: `${form.fullName} (${form.civilId})`, message: `ISSUE / INCIDENT REPORT\n━━━━━━━━━━━━━━━━━━━━━━━━━━\nCitizen: ${form.fullName}\nCivil ID: ${form.civilId}\n\nIssue Type: ${form.issueType}\nProvince: ${form.province}\nSeverity: ${form.severity}\n\nDescription:\n${form.description}\n\nDesired Outcome:\n${form.desiredOutcome||"Not specified."}` };
    ...
    <div className="min-h-screen bg-ottoman-red-950 relative">
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-6 py-12 space-y-8">
        ...
        <div className="space-y-2">
          ...
          <h1 className="font-serif text-3xl font-bold text-ivory-100">Report an Issue or Incident</h1>
          <p className="text-sm text-ivory-300/60 font-sans">This secure channel transmits your report directly to the Royal Diwan authorities.</p>
        </div>
        {status==="success"?(
          <div className="bg-emerald-900/40 border border-emerald-600/40 rounded-2xl p-8 text-center space-y-3">
            ...
            <p className="text-sm text-ivory-300/70 font-sans">Your report has been securely transmitted to the Royal Diwan.</p>
          ...
          <form onSubmit={handleSubmit} className="bg-ottoman-red-900/50 border border-brass-gold-700/20 rounded-2xl p-7 space-y-5">
            ...
            <FormField label="Issue Category" id="issueType" required>
              <select id="issueType" value={form.issueType} onChange={e=>setForm(f=>({...f,issueType:e.target.value}))} required className={inputClass}><option value="">Select issue type…</option>{ISSUE_TYPES.map(t=><option key={t}>{t}</option>)}</select>
            </FormField>
            <div className="grid grid-cols-2 gap-4">
              <FormField label="Province / Location" id="province" required>
                <select id="province" value={form.province} onChange={e=>setForm(f=>({...f,province:e.target.value}))} required className={inputClass}><option value="">Select province…</option>{PROVINCES.map(p=><option key={p}>{p}</option>)}</select>
              </FormField>
              <FormField label="Severity Level" id="severity" required>
                <select id="severity" value={form.severity} onChange={e=>setForm(f=>({...f,severity:e.target.value}))} required className={inputClass}><option value="">Select severity…</option><option>🟢 Low — General Concern</option><option>🟡 Medium — Needs Attention</option><option>🔴 High — Urgent Matter</option><option>⚫ Critical — Emergency</option></select>
              </FormField>
            </div>
            <FormField label="Detailed Description" id="description" required><textarea id="description" value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} rows={5} placeholder="Provide a clear, factual account of the issue…" required className={`${inputClass} resize-none`}/></FormField>
            <FormField label="Desired Outcome (optional)" id="desiredOutcome"><textarea id="desiredOutcome" value={form.desiredOutcome} onChange={e=>setForm(f=>({...f,desiredOutcome:e.target.value}))} rows={3} placeholder="What resolution are you requesting?" className={`${inputClass} resize-none`}/></FormField>
