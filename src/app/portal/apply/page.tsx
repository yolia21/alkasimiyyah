...
const POSITIONS = [
  "Provincial Governor — Wilayat Ghabaan",
  "Provincial Governor — Wilayat Jama'ah",
  "Deputy Vizier (Naib al-Wazir)",
  "Minister of Finance & Treasury",
  "Minister of Culture & Heritage",
  "Minister of Foreign Affairs",
  "Minister of Education",
  "Chief Justice (Qadi al-Qudat)",
  "Royal Archivist",
  "Overseas Cultural Attaché",
...
export default function ApplyPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      subject: `[Kasimid Portal] Government Application — ${citizen?.civilId ?? "Unknown"}`,
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-6 py-12 space-y-8">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-ivory-300/60 font-sans">
            Apply for an official role within the Royal Diwan, provincial administration, or overseas
            diplomatic service. Applications are reviewed by the Grand Vizier.
          </p>
        </div>
        ...
        {status === "success" ? (
          <div className="bg-emerald-900/40 border border-emerald-600/40 rounded-2xl p-8 text-center space-y-3">
            ...
            <p className="text-sm text-ivory-300/70 font-sans">
              Your application has been transmitted to the Grand Vizier&apos;s office for review. You will be contacted through your registered civil correspondence.
            </p>
          ...
          <form onSubmit={handleSubmit} className="bg-ottoman-red-900/50 border border-brass-gold-700/20 rounded-2xl p-7 space-y-5">
            ...
            <FormField label="Qualifications & Relevant Experience" id="qualifications" required>
              <textarea
                id="qualifications"
                value={form.qualifications}
                onChange={e => setForm(f => ({ ...f, qualifications: e.target.value }))}
                rows={4}
                placeholder="Describe your relevant skills, education, and prior roles…"
                required
                className={`${inputClass} resize-none`}
              />
            </FormField>

            <FormField label="Statement of Motivation" id="motivation" required>
              <textarea
                id="motivation"
                value={form.motivation}
                onChange={e => setForm(f => ({ ...f, motivation: e.target.value }))}
                rows={4}
                placeholder="Why do you wish to serve the Kasimid Sultanate in this capacity?"
                required
                className={`${inputClass} resize-none`}
              />
            </FormField>
